...
class _TestPubRec is UnitTest
  ...
  fun _test_mqtt311(h: TestHelper) ? =>
    ...
    let buf = MqttPubRec.encode(consume origin, None, MqttVersion311)
  ...
  fun _test_mqtt5(h: TestHelper) ? =>
    ...
    | (MqttDecodeDone, let packet: MqttControlPacketType val, _) =>
      ...
      | let pkt: MqttPubRecPacket val =>
        h.assert_eq[U16 val](pkt.packet_identifier, 65535)
        try h.assert_eq[U8 val]((pkt.reason_code as MqttPubRecReasonCode)(), MqttSuccess()) else h.fail("Expect reason-code to be MqttSuccess but got None") end
