...
class _TestPubComp is UnitTest
  ...
  fun _test_mqtt311(h: TestHelper) ? =>
    let origin = MqttPubCompPacket(
      where
      packet_identifier' = 65535
    )

    let buf = MqttPubComp.encode(origin, None, MqttVersion311)
    match MqttDecoder(buf, MqttVersion311) ?
    | (MqttDecodeDone, let packet: MqttControlPacketType) =>
      match packet
      | let pkt: MqttPubCompPacket =>
  ...
  fun _test_mqtt5(h: TestHelper) ? =>
    ...
    | (MqttDecodeDone, let packet: MqttControlPacketType) =>
      ...
      | let pkt: MqttPubCompPacket =>
        h.assert_eq[U16](pkt.packet_identifier, 65535)
        try h.assert_eq[U8]((pkt.reason_code as MqttPubCompReasonCode)(), MqttSuccess()) else h.fail("Expect reason-code to be MqttSuccess but got None") end
