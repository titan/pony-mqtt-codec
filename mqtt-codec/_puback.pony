...
class _TestPubAck is UnitTest
  ...
  fun _test_mqtt5(h: TestHelper) ? =>
    ...
    | (MqttDecodeDone, let packet: MqttControlPacketType) =>
      ...
      | let pkt: MqttPubAckPacket =>
        h.assert_eq[U16](pkt.packet_identifier, 65535)
        try h.assert_eq[U8]((pkt.reason_code as MqttPubAckReasonCode)(), MqttSuccess()) else h.fail("Expect reason-code to be MqttSuccess but got None") end
