...
type MqttPubCompReasonCode is (MqttSuccess | MqttPacketIdentifierNotFound | MqttUnspecifiedError)

class MqttPubCompPacket
  ...
  * mqtt-3.1
  ...
  let reason_code: (MqttPubCompReasonCode val | None)
  """
  PUBACK Reason Code
  ...
  * mqtt-5
  ...
  new iso create(
      ...
      reason_code': (MqttPubCompReasonCode val | None) = None,
...
primitive MqttPubCompDecoder
  fun apply(reader: Reader, header: U8 box, remaining: USize box, version: MqttVersion box = MqttVersion5): MqttDecodeResultType[MqttPubCompPacket val] val ? =>
    (let packet_identifier: U16, _) = MqttTwoByteInteger.decode(reader) ?
    if \likely\ version() == MqttVersion5() then
      let reason_code: (MqttPubCompReasonCode | None) =
        match reader.u8() ?
...
primitive MqttPubCompMeasurer
  fun variable_header_size(data: MqttPubCompPacket box, maximum_packet_size: (USize box | None) = None, version: MqttVersion box = MqttVersion5): USize val =>
  ...
  fun properties_size(data: MqttPubCompPacket box, maximum_packet_size: (USize box | None) = None): USize val =>
    var size: USize = 0
...
primitive MqttPubCompEncoder
  fun apply(data: MqttPubCompPacket box, maximum_packet_size: (USize box | None) = None, version: MqttVersion box = MqttVersion5): Array[U8] val =>
    var maximum_size: (USize | None) = None
    var remaining: USize = 0
    match maximum_packet_size
    | let maximum_packet_size': USize box =>
      var maximum: USize = maximum_packet_size' - 1 - 1
      remaining = MqttPubCompMeasurer.variable_header_size(data, maximum, version)
      var remaining_length = MqttVariableByteInteger.size(remaining.ulong())
      maximum = maximum - remaining_length
      var delta: USize = 0
      repeat
        maximum = maximum - delta
        let remaining': USize = MqttPubCompMeasurer.variable_header_size(data, maximum, version)
      ...
      remaining = MqttPubCompMeasurer.variable_header_size(data, None, version)
    end
    ...
    if \likely\ version() == MqttVersion5() then
      ...
      | let reason_code: MqttPubCompReasonCode box =>
        buf.push(reason_code())
      end

      var properties_length: USize = MqttPubCompMeasurer.properties_size(data, maximum_size)
