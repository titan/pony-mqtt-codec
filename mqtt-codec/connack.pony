  ...
  : MqttConnAckPacket? =>
    ...
    if \likely\ version == MqttVersion5 then
      ...
      while decoded_length < property_length do
        ...
        | _MqttMaximumQoS() =>
          (let maximum_qos', let maximum_qos_size) = _MqttMaximumQoS.decode(buf, offset' + decoded_length)?
          maximum_qos = maximum_qos'
          decoded_length = decoded_length + maximum_qos_size
        | _MqttRetainAvailable() =>
          (let retain_available', let retain_available_size) = _MqttRetainAvailable.decode(buf, offset' + decoded_length)?
        ...
        | _MqttAssignedClientIdentifier() =>
          (let assigned_client_identifier', let assigned_client_identifier_size) = _MqttAssignedClientIdentifier.decode(buf, offset' + decoded_length)?
        ...
        | _MqttUserProperty() =>
          ((let key, let value), let user_property_size) = _MqttUserProperty.decode(buf, offset' + decoded_length)?
        ...
        | _MqttWildcardSubscriptionAvailable() =>
          (let wildcard_subscription_available', let wildcard_subscription_available_size) = _MqttWildcardSubscriptionAvailable.decode(buf, offset' + decoded_length)?
        ...
        | _MqttSubscriptionIdentifierAvailable() =>
          (let subscription_identifier_available', let subscription_identifier_available_size) = _MqttSubscriptionIdentifierAvailable.decode(buf, offset' + decoded_length)?
        ...
        | _MqttSharedSubscriptionAvailable() =>
          (let shared_subscription_available', let shared_subscription_available_size) = _MqttSharedSubscriptionAvailable.decode(buf, offset' + decoded_length)?
        ...
        | _MqttServerKeepAlive() =>
          (let server_keep_alive', let server_keep_alive_size) = _MqttServerKeepAlive.decode(buf, offset' + decoded_length)?
          server_keep_alive = server_keep_alive'
          decoded_length = decoded_length + server_keep_alive_size
        | _MqttResponseInformation() =>
          (let response_information', let response_information_size) = _MqttResponseInformation.decode(buf, offset' + decoded_length)?
        ...
        | _MqttServerReference() =>
          (let server_reference', let server_reference_size) = _MqttServerReference.decode(buf, offset' + decoded_length)?
