...
primitive MqttUtf8StringPair
  ...
  fun encode(buf: Array[U8], data: (String box, String box)): USize val =>
    let key = data._1
    let value = data._2
...
primitive MqttVariableByteInteger
  ...
  fun decode_array(buf: Array[U8] val, offset: USize = 0): (ULong, USize) ? =>
    var x: ULong = 0
  ...
  fun decode_reader(reader: Reader): (ULong, USize) ? =>
    var x: ULong = 0
    var idx: USize = 0
    var byte: U8
    var multiplier: ULong = 0
