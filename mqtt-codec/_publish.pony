  ...
  fun _test_mqtt311(h: TestHelper) ? =>
    ...
    | (MqttDecodeDone, let packet: MqttControlPacketType val, _) =>
      ...
      | let pkt: MqttPublishPacket val =>
        ...
        try h.assert_eq[U16 val](pkt.packet_identifier as U16, 0) else h.fail("Expect packet-identifier to be 0 but got None") end
  ...
  fun _test_mqtt5(h: TestHelper) ? =>
    ...
    | (MqttDecodeDone, let packet: MqttControlPacketType val, _) =>
      ...
      | let pkt: MqttPublishPacket val =>
        ...
        h.assert_eq[Bool val](pkt.retain, true)
        h.assert_eq[String val](pkt.topic_name, "")
        try h.assert_eq[U16 val](pkt.packet_identifier as U16 val, 0) else h.fail("Expect packet-identifier to be 0 but got None") end
        ...
        try h.assert_eq[U16 val](pkt.topic_alias as U16 val, 65535) else h.fail("Expect topic-alias to be 65535 but got None") end
        ...
        try h.assert_eq[ULong](pkt.subscription_identifier as ULong val, 127) else h.fail("Expect subscription-identifier to be 127 but got None") end
